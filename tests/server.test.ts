...
function createTestServer(): McpServer {
  ...
  server.registerTool(
    'get_current_user',
    { description: "Get the authenticated Calendly user's profile", annotations: { readOnlyHint: true } },
    (): Promise<CallToolResult> => getCurrentUser(mockClient),
  );

  server.registerTool(
    'list_event_types',
    { description: "List the user's Calendly event types", inputSchema: ListEventTypesSchema.shape, annotations: { readOnlyHint: true } },
    (args): Promise<CallToolResult> => listEventTypes(mockClient, USER_URI, args),
  );

  server.registerTool(
    'list_scheduled_events',
    { description: 'List scheduled Calendly events with optional filters', inputSchema: ListScheduledEventsSchema.shape, annotations: { readOnlyHint: true } },
    (args): Promise<CallToolResult> => listScheduledEvents(mockClient, USER_URI, args),
  );

  server.registerTool(
    'get_event',
    { description: 'Get details of a specific scheduled event', inputSchema: GetEventSchema.shape, annotations: { readOnlyHint: true } },
    (args): Promise<CallToolResult> => getEvent(mockClient, args),
  );

  server.registerTool(
    'list_invitees',
    { description: 'List invitees for a scheduled event', inputSchema: ListInviteesSchema.shape, annotations: { readOnlyHint: true } },
    (args): Promise<CallToolResult> => listInvitees(mockClient, args),
  );

  server.registerTool(
    'cancel_event',
    { description: 'Cancel a scheduled Calendly event', inputSchema: CancelEventSchema.shape, annotations: { destructiveHint: true } },
    (args): Promise<CallToolResult> => cancelEvent(mockClient, args),
  );

  server.registerTool(
    'check_availability',
    { description: 'Get available time slots for an event type', inputSchema: CheckAvailabilitySchema.shape, annotations: { readOnlyHint: true } },
...
describe('calendly MCP server', () => {
  ...
  it('list_event_types returns paginated event types', async () => {
    mocks.listEventTypes.mockResolvedValueOnce(paginatedResponse([mockEventType]));

    const result = await client.callTool({ name: 'list_event_types', arguments: {} });
    const parsed = JSON.parse(getText(result));
  ...
  it('list_scheduled_events returns paginated events', async () => {
    mocks.listScheduledEvents.mockResolvedValueOnce(paginatedResponse([mockScheduledEvent]));

    const result = await client.callTool({ name: 'list_scheduled_events', arguments: {} });
    const parsed = JSON.parse(getText(result));
  ...
  it('list_scheduled_events forwards status filter to client', async () => {
    ...
    await client.callTool({ name: 'list_scheduled_events', arguments: { status: 'canceled' } });
  ...
  it('get_event returns formatted event details', async () => {
    ...
    const result = await client.callTool({ name: 'get_event', arguments: { event_uuid: 'EV123' } });
    const parsed = JSON.parse(getText(result));
  ...
  it('list_invitees returns paginated invitees', async () => {
    ...
    const result = await client.callTool({ name: 'list_invitees', arguments: { event_uuid: 'EV123' } });
    const parsed = JSON.parse(getText(result));
  ...
  it('cancel_event returns success message containing the event uuid', async () => {
    ...
    const result = await client.callTool({ name: 'cancel_event', arguments: { event_uuid: 'EV123' } });
  ...
  it('cancel_event forwards optional reason to client', async () => {
    ...
    await client.callTool({ name: 'cancel_event', arguments: { event_uuid: 'EV123', reason: 'Schedule conflict' } });

    expect(mocks.cancelEvent).toHaveBeenCalledWith('EV123', 'Schedule conflict');
  });
  ...
  it('propagates CalendlyApiError from get_current_user', async () => {
    ...
    mocks.getCurrentUser.mockRejectedValueOnce(
      new CalendlyApiError(401, 'Unauthorized', 'Calendly API error 401: Invalid token'),
    );
  ...
  it('returns error when check_availability is missing required fields', async () => {
    const result = await client.callTool({
      name: 'check_availability',
      arguments: { event_type_uri: 'https://api.calendly.com/event_types/ET123' },
    });
    expect(result.isError).toBe(true);
  });

  it('returns error when list_invitees is called without event_uuid', async () => {
    const result = await client.callTool({ name: 'list_invitees', arguments: {} });
    expect(result.isError).toBe(true);
  });
});
