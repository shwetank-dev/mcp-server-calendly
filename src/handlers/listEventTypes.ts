...
export async function listEventTypes(
  ...
  try {
    const result = await client.listEventTypes(userUri, args.count, args.page_token);
    return { content: [{ type: 'text', text: JSON.stringify(formatPaginated(result, formatEventType), null, 2) }] };
  } catch (e) {
