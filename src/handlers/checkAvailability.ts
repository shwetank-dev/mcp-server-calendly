...
export async function checkAvailability(
  ...
  try {
    const slots = await client.getAvailableTimes(args.event_type_uri, args.start_time, args.end_time);
    return { content: [{ type: 'text', text: JSON.stringify(slots.map(formatAvailableTime), null, 2) }] };
  } catch (e) {
